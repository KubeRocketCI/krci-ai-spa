...
/**
 * Data-specific card content component
 * Follows template card content pattern for consistency
 */

import { memo, useState } from 'react';
import { Copy, Check } from 'lucide-react';
import { ThemedInlineCode } from '@/components/ui/themed-inline-code';
import { getDataTextClasses, DATA_DESIGN_TOKENS } from '@/lib/data-design-tokens';
...
export const DataCardContent = memo(function DataCardContent({
  dataFile,
  variant = 'feature',
}: DataCardContentProps) {
  const textClasses = getDataTextClasses();

  // Variant-specific display logic
  const shouldShowPath = variant !== 'compact';
  const descriptionLineClamp =
    variant === 'compact' ? 1 : DATA_DESIGN_TOKENS.content.lineClamp.description;

  return (
    <div className="space-y-4">
      {/* Description Section - variant-aware line clamping */}
      <div className="min-h-10 flex items-start">
        <p className={textClasses.description} title={dataFile.description}>
          <span
            className={`block ${DATA_DESIGN_TOKENS.content.lineHeight.description} ${DATA_DESIGN_TOKENS.content.maxHeight.description}`}
            style={{
              display: '-webkit-box',
      ...
      {shouldShowPath && (
        <div className="mt-2 pt-1.5 border-t border-slate-300/60 dark:border-white/20">
          <div className="flex items-center justify-end gap-2 min-h-[1.5rem]">
            ...
            <TruncatedPath path={dataFile.path} />
          </div>
        </div>
      )}
    </div>
